...
`;let o=0;const k=document.getElementById("track"),d=document.querySelectorAll(".game-card"),b=document.getElementById("hero-bg"),w=document.getElementById("d-title"),L=document.getElementById("d-desc"),E=document.getElementById("d-tags"),p=document.getElementById("play-btn");function x(){return window.innerWidth>=1024?{cardWidth:150,gap:20,activeGrowth:0}:{cardWidth:140,gap:20,activeGrowth:20}}function c(e){d.forEach(a=>a.classList.remove("active")),d[e]&&d[e].classList.add("active");const t=x(),s=e*(t.cardWidth+t.gap);k.style.transform=`translateX(-${s}px)`;const n=r[e];w.innerText=n.title,L.innerText=n.desc,E.innerHTML=n.tags.map(a=>`<span class="tag-badge">${a}</span>`).join(""),p.textContent=n.locked?"SOON":"PLAY NOW",b.innerHTML=`<img src="${n.hero}" class="hero-image" style="animation: fadeIn 0.4s">`}let f=0,m=!1;const v=document.getElementById("carousel-container");v.addEventListener("touchstart",e=>{f=e.touches[0].clientX,m=!0},{passive:!0});v.addEventListener("touchend",e=>{if(!m)return;const t=f-e.changedTouches[0].clientX;Math.abs(t)>30&&(t>0&&o<r.length-1?o++:t<0&&o>0&&o--,c(o)),m=!1});const u=document.getElementById("game-container"),S=document.getElementById("close-btn");let l=null;function h(){const e=r[o];if(e.locked||!e.path)return;const t=`${y}${e.path}`,s=document.createElement("iframe");s.src=t,s.sandbox="allow-scripts allow-same-origin allow-popups allow-forms allow-pointer-lock",s.allow="autoplay; fullscreen",u.appendChild(s),u.style.display="block",l=s,s.onload=()=>s.contentWindow.focus()}d.forEach((e,t)=>{e.addEventListener("click",()=>{o=t,c(o),window.innerWidth>=1024&&h()})});p.addEventListener("click",h);S.addEventListener("click",()=>{l&&(l.src="about:blank",l.remove(),l=null),u.style.display="none",window.history.pushState({},document.title,window.location.pathname)});function P(){const t=new URLSearchParams(window.location.search).get("id");if(t){const s=r.findIndex(n=>n.id.toLowerCase()===t.toLowerCase());s!==-1?(o=s,c(o),r[o].locked||h()):c(0)}else c(0)}P();window.addEventListener("resize",()=>c(o));
